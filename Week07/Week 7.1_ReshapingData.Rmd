...
newtable4b <- pivot_longer(table4b, cols = c('1999', '2000'), names_to = "year", values_to = "population")
newtable4b

right_join(newtable4a, newtable4b) |>
  mutate("rate per 100k" = (cases/population) * 100000)
...
# Write and submit code here!
table1
pivot_wider(table1, names_from = year, values_from = c(cases, population))
```
...
# Write and submit code here!
billboard |>
  pivot_longer(cols = c(wk1:wk76), names_to = "week", values_to = "ranking")
```


2. Which function should you use to combine the name of the `artist` with the name of the `track`, separated by `:`? Try it!

```{r}
# Write and submit code here!
billboard |>
  unite(name, artist, track, sep = ":")
```


3. In which format should `billboard` be if we wanted to count how many songs per artist entered the Billboard top 100 in 2000? Try it! Which artists had more than one song?
